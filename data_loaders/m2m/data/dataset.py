...
"""For use of training motion text motion (m2m) matching model, and evaluations"""
class Motion2MotionDatasetV2(data.Dataset):
    def __init__(self, opt, mean, std, split_file, w_vectorizer):
        self.opt = opt
        self.w_vectorizer = w_vectorizer
        self.max_length = 20
        self.pointer = 0
        self.max_motion_length = opt.max_motion_length
        min_motion_len = 40 if self.opt.dataset_name =='m2m' else 24
...
class M2M(data.Dataset):
    def __init__(self, mode, datapath='./dataset/m2m_opt.txt', split="train", **kwargs):
        self.mode = mode
        
        self.dataset_name = 'm2m'
        self.dataname = 'm2m'
        ...
        if mode == 'gt':
            ...
            # used by our models
            self.mean = np.load("dataset/HumanML3D/Mean.npy")
            self.std = np.load("dataset/HumanML3D/Std.npy")
            # self.mean = np.load(pjoin(opt.data_root, 'Mean.npy'))
            # self.std = np.load(pjoin(opt.data_root, 'Std.npy'))
        ...
        self.split_file = pjoin(opt.data_root, f'{split}.txt')
        if mode == 'text_only':
            self.m2m_dataset = TextOnlyDataset(self.opt, self.mean, self.std, self.split_file)
        else:
            self.w_vectorizer = WordVectorizer(pjoin(abs_base_path, 'glove'), 'our_vab')
            self.m2m_dataset = Motion2MotionDatasetV2(self.opt, self.mean, self.std, self.split_file, self.w_vectorizer)
            self.num_actions = 1 # dummy placeholder

        assert len(self.m2m_dataset) > 1, 'You loaded an empty dataset, ' \
                                          'it is probably because your data dir has only texts and no motions.\n' \
                                          'To train and evaluate MDM you should get the FULL data as described ' \
                                          'in the README file.'

    def __getitem__(self, item):
        return self.m2m_dataset.__getitem__(item)

    def __len__(self):
        return self.m2m_dataset.__len__()